...
public:
	...
	Perturbation(ParameterHandler &prm, Materials &materials,
...
//  std::vector<unsigned int> assem_per_dim;
	...
	void move_bar_volume_homogenized(unsigned int bar_plant_pos, double bar_pos,
			unsigned int mat_bar, unsigned int bar);

	void move_bar_flux_weighting(unsigned int bar_plant_pos, double bar_pos,
			unsigned int mat_bar, unsigned int bar);
	...
	void modify_xsec(double sim_time, unsigned int nmat,
