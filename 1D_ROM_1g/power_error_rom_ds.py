...
markers = ['-', 'o', '*', '^', 'v']
labels = ['FOM','POD-3', 'POD-5', 'LUPOD-3', 'LUPOD-5' ]
out_files =['1D_ROM_td.out', 
            '1D_ROM_pod_dyn3.out',
            '1D_ROM_pod_dyn5.out',
            '1D_ROM_pod_dyn3_LUPOD.out',
            '1D_ROM_pod_dyn5_LUPOD.out']

#             
# labels = ['FOM','POD-10']
# out_files =['1D_ROM_td.out', '1D_ROM_pod_sta10.out']
...
for i in range(len(out_files)):
    ...
    _, _, local_pow = parse_time_file(out_files[i]+ '.time')
...
fig.savefig('1D_ROM_1g.pdf', format='pdf')
