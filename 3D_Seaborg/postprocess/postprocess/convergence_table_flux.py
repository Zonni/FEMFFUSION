...
from utils import get_n_dofs, get_flux
import numpy as np
...
if problem == 'Langenbuch':
    ...
    file_r = '../2D_IAEA_noise/2D_IAEA.parcs'
#    file_r = '../2D_IAEA_noise/2D_IAEA_ref.ref'
...
eig_r = round(readEig(file_r)[0], 5)
...
fl1_ref = np.array(get_flux(file_r, 1))
norm = max(fl1_ref)
fl1_ref =  fl1_ref / norm
fl2_ref = np.array(get_flux(file_r, 2))/norm
...
for i in range(len(files)):
    print(latexRow([i, dofs[i], err_eigs[i], err_eigs[i], fl1_mean[i], fl2_mean[i]]))
print(eig_r)
