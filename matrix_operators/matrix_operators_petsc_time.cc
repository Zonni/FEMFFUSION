...
#include "matrix_operators_petsc_time.h"
    ...
    const std::vector<double> &albedo_factors)
  {
    ...
    for (unsigned int g = 0; g < n_groups; g++)
    {
      ...
      for (unsigned int mat = 0; mat < n_mats; mat++)
      {
        ...
        coeffs_val[g][mat] += 1
                              / (this->delta_t * materials.get_velocitiy(mat, g));
    ...
    for (; cell != endc; ++cell)
      if (cell->is_locally_owned())
      {
        ...
        for (unsigned int gi = 0; gi < materials.get_n_groups(); ++gi)
        {
          ...
          coeff_cell_val += 1
                            / (this->delta_t * materials.get_velocitiy(mat, gi));
    ...
    const Materials &materials)
  {
    ...
    for (; cell != endc; ++cell)
      if (cell->is_locally_owned())
      {
        ...
        {
          // Get the material coefficients:
          vel = 1. / materials.get_velocitiy(mat, gi);
