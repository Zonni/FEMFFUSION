...
#include "matrix_operators_spn_time.h"
            ...
            for (unsigned int mj = 0; mj < this->n_moments; ++mj)
            {
              ...
              if (gi == gj)
              {
                if (mi == mj)
                {
                  ...
                  for (unsigned int m3 = 0; m3 < this->n_moments; ++m3)
                    sigma += sp_coeff[m3][mi][mj] / this->delta_t
                             / materials.get_velocitiy(mat, gi);

                  sigma +=
                           -sp_coeff[0][mi][mj]
                           * (1
                              - materials.get_delayed_fraction_sum(
                                mat))
                           * materials.get_prompt_spectra(
                             mat, gi)
                           * materials.get_nu_sigma_f(gi,
                             mat);
                ...
                else
                {
                  sigma = sp_coeff[0][mi][mj]
                          * materials.get_sigma_r(gi, mat);
                  for (unsigned int m3 = 1; m3 < this->n_moments;
                      ++m3)
                  {
                    sigma += sp_coeff[m3][mi][mj]
                             * (materials.get_sigma_r(gi, mat)
