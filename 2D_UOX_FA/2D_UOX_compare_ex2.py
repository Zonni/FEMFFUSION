...
labels = ['SP1', 'SP3', 'SP5', 'SP7', 'S64 (Yi, 2021)']
style = ['*-', '.-', '-^', '--+', '-']
...
for i in range(n_files):
    ...
    # else:
    #     print('SN postprocess')
    #     rel_g1.append(amp_g1[i])
    #     rel_g2.append(amp_g2[i])
...
for i in range(n_files):
    ax1.plot(x, sta_g1_line[i]/np.mean(sta_g1_line[i]), style[i], label=labels[i])
ax1.grid(True)
ax1.legend(loc='best')
ax1.set_xlabel("x (cm)")
...
for i in range(n_files):
    ax1.plot(x, sta_g2_line[i]/np.mean(sta_g2_line[i]), style[i], label=labels[i])
