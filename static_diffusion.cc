...
template <int dim, int n_fe_degree>
      ...
      perturbation(prm, materials, dof_handler, verbose_cout)
  {
    ...
    if (prm.get("Bar_Filename") != "no.bar" or type_perturbation == "Rods")
    {
      ...
      AssertRelease(type_perturbation == "Rods", "Type_Perturbation must be Rods");
    ...
    for (; cell != endc; ++cell)
      if (cell->is_locally_owned())
      {
        ...
        if (show_materials)
        {
          ...
          cout << std::endl;
          cout << " Sigma_A " << materials.get_sigma_r(0, mat) << std::endl;
        }
 ...
 */
template <int dim, int n_fe_degree>
  void StaticDiffusion<dim, n_fe_degree>::load_static_calculation (std::string &file)
  {
    ...
    for (std::string line; getline(input, line);)
    {
      ...
      else if (keyword == "phi")
      {
        ...
        cout << "     phi: " << phi_critic[0] << " ... " << phi_critic.back()
             << std::endl;
      }
    }
    // Make critical
 ...
 */
template <int dim, int n_fe_degree>
  void StaticDiffusion<dim, n_fe_degree>::save_static_calculation (std::string &file)
  {
...
template <int dim, int n_fe_degree>
  void
  StaticDiffusion<dim, n_fe_degree>::solve_eps ()
  {
    EPSSolver<dim, n_fe_degree> *solver = NULL;

    if (t_modes == "lambda")
      solver = new EPSSolver<dim, n_fe_degree>(solver_type,
        T, F, n_eigenvalues, timer, show_eps_convergence,
        verbose_cout.is_active(), tria, dof_handler, fe);
    else if (t_modes == "alpha")
      solver = new EPSSolver<dim, n_fe_degree>(solver_type,
        A, V, n_eigenvalues, timer, show_eps_convergence,
        verbose_cout.is_active(), tria, dof_handler, fe);
    else if (t_modes == "gamma")
      solver = new EPSSolver<dim, n_fe_degree>(solver_type,
        L, G, n_eigenvalues, timer, show_eps_convergence,
        verbose_cout.is_active(), tria, dof_handler, fe);

    // Select some solver options
    if (solver_type == "slepc_2g" or solver_type == "ks" or solver_type == "slepc_7g")
      p_init = false;
      ...
      for (unsigned int eig = 0; eig < n_eigenvalues; eig++)
      {
        ...
        for (; cell != endc; ++cell)
        {
          ...
          for (unsigned int g = 0; g < n_groups; ++g)
          {
            ...
            fe_values.get_function_values(phi_serial[eig].block(g), local_phi);
        ...
        print_cell_distribution_in_file(dim, power_per_assembly[eig], assem_per_dim,
          out_stream, materials, "Neutron Power\n", false, presicion);
...
template <int dim, int n_fe_degree>
  void StaticDiffusion<dim, n_fe_degree>::output_results () const
  {
    ...
    {
      mat_id_cells[c] = static_cast<double>(materials.get_material_id<dim>(cell) + 1);
    }
