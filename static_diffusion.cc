...
StaticDiffusion<dim, n_fe_degree>::StaticDiffusion(ParameterHandler &prm,
		std::string input_file, const bool verbose, const bool silent,
		const bool _to_init, std::string modes) :
		comm(PETSC_COMM_WORLD), n_mpi_processes(
				Utilities::MPI::n_mpi_processes(comm)), this_mpi_process(
				Utilities::MPI::this_mpi_process(comm)), n_local_cells(
				numbers::invalid_unsigned_int), verbose_cout(std::cout,
				verbose and this_mpi_process == 0), materials_cout(std::cout,
				false), // No verbose for materials
		cout(std::cout, !silent and this_mpi_process == 0), to_init(_to_init), n_groups(
				prm.get_integer("Energy_Groups")), fe(
				QGaussLobatto<1>(n_fe_degree + 1)), tria(comm), dof_handler(
				tria), T(comm, dof_handler, constraints), F(comm, dof_handler,
				constraints), L(comm, dof_handler, constraints), G(comm,
				dof_handler, constraints), A(comm, dof_handler, constraints), V(
				comm, dof_handler, constraints), input_file(input_file), materials(
				materials_cout), perturbation(prm, materials, dof_handler,
				verbose_cout) {
...
void StaticDiffusion<dim, n_fe_degree>::make_rectangular_grid() {
	...
	Point<dim> p1 = (
			(dim == 1) ?
					Point<dim>(0.0) :
					((dim == 2) ?
							Point<dim>(0.0, 0.0) : Point<dim>(0.0, 0.0, 0.0)));
	if (dim == 1)
...
//template <int dim, int n_fe_degree>
//  void
//  StaticDiffusion<dim, n_fe_degree>::parse_bar_file (std::string BarFile)
//  //
//  {
...
//        parseMultilineVector(input, assem_per_dim[1], bars_position, true);
//
//        if (verbose_cout.is_active())
...
void StaticDiffusion<dim, n_fe_degree>::make_dofs() {
	...
	//    DoFRenumbering::Cuthill_McKee(dof_handler, false);
	//    verbose_cout << "   Renumbering reversed_Cuthill_McKee. " << std::flush;

	//    DoFRenumbering::boost::minimum_degree(dof_handler);
	//    verbose_cout << "   Renumbering minimum_degree. " << std::flush;
...
void StaticDiffusion<dim, n_fe_degree>::assemble_system_lambda() {
	...
	cout << "   Memory consumption of matrix elements "
			<< memory_consumption * 1e-6 << " MB" << std::endl;
	...
	if (this_mpi_process == 0) {
		...
		FEValues<dim> fe_values(fe, quadrature_formula,
				update_values | update_quadrature_points
						| update_volume_elements | update_JxW_values);
 ...
 */
template<int dim, int n_fe_degree>
void StaticDiffusion<dim, n_fe_degree>::postprocess2() {
	std::vector<double> norm = std::vector<double>(n_eigenvalues);
	...
	if (this_mpi_process == 0) {
		...
		for (unsigned int eig = 0; eig < n_eigenvalues; eig++) {
			...
			std::cout << "POWER TOT " << norm[eig] << std::endl;
