...
#ifndef TIME_COMPUTATION_H_
#define TIME_COMPUTATION_H_
...
template <int dim, int n_fe_degree>
  class TimeNeutronDiffusion
  {
    ...
    // Matrices
    MassMatrixTime<dim, n_fe_degree> Rfree;
    FisionMatrixTime<dim, n_fe_degree> Mfree;
    SystemMatrixTime<dim, n_fe_degree> Tfree;
    ...
    PetscInt its, old_its;
    PetscInt totalits;

    std::vector<KSP> ksp_blocks;
    std::vector<PETScWrappers::MPI::BlockVector> vectors_sols;
