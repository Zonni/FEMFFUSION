...
#include "../../include/io/input_complex_pert.h"
  ...
  std::vector<std::vector<std::vector<double>> > _new_xsec_s)
{
  ...
  n_groups = _n_groups;
  materials_vector = _materials_vector;
  // TODO Check if this function is necesary
  // 0: sigma_tr, 1: sigma_r, 2: nusigmaf, 3: sigma_f,
  sigma_tr = _new_xsec[0];
  sigma_r = _new_xsec[1];
  ...
  const unsigned int user_index)
{
  ...
  for (unsigned int g = 0; g < n_groups; ++g)
  {
    sigma_tr[g][new_mat] = frac * sigma_tr[g][mat_bar]
                           + (1 - frac) * sigma_tr[g][mat_no_bar];
    ...
    for (unsigned int p = 0; p < n_precursors; p++)
      delayed_spectra[new_mat][p][g] = frac * delayed_spectra[mat_bar][p][g]
  ...
  for (unsigned int p = 0; p < n_precursors; ++p)
  {
    ...
    delayed_decay_constants[new_mat][p] = frac * delayed_decay_constants[mat_bar][p]
  ...
  const unsigned int user_index)
{
  ...
  for (unsigned int g = 0; g < n_groups; ++g)
  {
    ...
    sigma_tr[g][new_mat] = frac * sigma_tr[g][mat_bar] * flux_bar
                           + (1 - frac) * sigma_tr[g][mat_no_bar] * flux_nobar;
    sigma_tr[g][new_mat] /= den_flux;
  ...
  // Create new position in materials_vector if necesary
  AssertIndexRange(user_index, materials_vector.size()+1);
  if (user_index == materials_vector.size())
    materials_vector.push_back(new_mat);
  else
...
void Materials::create_new_added_mat (
  ...
  const unsigned int user_index)
{
  AssertRelease(n_groups == 2, "create_new_added_mat Implemented to n_groups");
  Assert(frac >= 0.0 and frac <= 1.0,
...
// If necessary add a reserve space for a new material
  ...
  for (unsigned int g = 0; g < n_groups; ++g)
  {
    sigma_tr[g][new_mat] = sigma_tr[g][mat_no_bar];
 ...
 *
 */
void Materials::remove_perturbation_xsec (
  std::string xsec_type,
  double &coeff)
{
 ...
 *  This functions parses the .xsec file completing XS data
      ...
      for (unsigned int mat = 0; mat < n_mats; mat++)
      {
        ...
        for (unsigned int g = 0; g < n_groups; g++)
        {
          ...
          AssertRelease(!iss.fail(),
            ...
            + ".");
          sigma_tr[g][mat] = num;
          verbose_cout << "      sigma_tr_" << g + 1 << " = " << num
                       << std::endl;
          // chi
          iss >> num;
          AssertRelease(!iss.fail(),
          ...
          for (unsigned int to_group = 0; to_group < n_groups; to_group++)
          {
            iss >> num;
            AssertRelease(!iss.fail(),
              "There are not enough (well) XSEC specified in mat "
...
//      lambda_prec.resize(n_precursors);
  ...
  // Compute sigma_r and sigma_t using the approximation sigma_tr=sigma_t
  sigma_r.resize(n_groups, std::vector<double>(n_mats));
  sigma_t.resize(n_groups, std::vector<double>(n_mats));
  for (unsigned int mat = 0; mat < n_mats; mat++)
    for (unsigned int g = 0; g < n_groups; g++)
    {
      sigma_t[g][mat] = sigma_tr[g][mat];
      sigma_r[g][mat] = sigma_tr[g][mat] - sigma_s[g][g][mat];
    }
  verbose_cout << "Done!" << std::endl;
...
void Materials::parse_forest_xs (const std::string &xml_file)
{
  ...
  for (unsigned int mat = 0; mat < n_mats; ++mat)
  {
    ...
    for (unsigned int g = 0; g < n_groups; ++g)
    {
      ...
      AssertRelease(chi_computed - chi[g][mat] < 1e-10,
        "The relation between the spectral is not satisfied.");
    }
  }
  ...
  const std::vector<unsigned int> geo_ps)
{
  ...
  sigma_tr.resize(n_groups);
  sigma_tr[0].reserve(n_mats);
  sigma_tr[1].reserve(n_mats);
  sigma_r.resize(n_groups);
  sigma_r[0].reserve(n_mats);
  sigma_r[1].reserve(n_mats);
  ...
  for (std::string line; getline(input, line);)
  {
    ...
    else if (keyword == "D1")
    {
      for (unsigned int j = 0; j < n_lines; j++)
      {
        ...
        std::istringstream iss2(line);
        iss2 >> str;
        if (lower_case(str) == "plane"
            || lower_case(str) == "plano"
        ...
        for (unsigned int k = 0; k < n_cell_per_line[line_num - 1]; k++)
        {
          iss2 >> num;
          sigma_tr[0].push_back(1.0 / (3.0 * num));
        }
      }
    ...
    else if (keyword == "D2")
    {
      for (unsigned int j = 0; j < n_lines; j++)
      {
        ...
        {
          iss2 >> num;
          sigma_tr[1].push_back(1.0 / (3.0 * num));
        }
      }
    }
    ...
    else if (keyword == "SGNUF2")
    {
      for (unsigned int j = 0; j < n_lines; j++)
      {
        ...
        if (lower_case(str) == "plane"
            or lower_case(str) == "plano"
            or lower_case(str) == "PLANE")
        {
          j--;
          continue;
        }
