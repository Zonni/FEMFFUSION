...
//    }
    if (matrixfree_type == full_allocated)
    {
      ...
      for (unsigned int g = 0; g < n_groups; g++)
        sp[g].reinit(n_LUPOD_points, this->dof_handler.n_dofs(),
        ...
        for (unsigned int g = 0; g < n_groups; g++)
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
          {
            ...
            if (point_index != static_cast<unsigned int>(-1))
            {
              ...
              {
                this->sp[g].add(point_index, dofs_on_this_cell[j]);
              }
            }
          }
      }
...
//          print_vector(local_row_indices);
          ...
          for (unsigned int gj = 0; gj < materials.get_n_groups(); ++gj)
          {
            ...
            if (gi == gj)
            {
              ...
              this->constraints.distribute_local_to_global(cell_L,
                local_row_indices,
                local_dof_indices,
...
//    {
    ...
    for (unsigned int g = 0; g < n_groups; g++)
      sp[g].reinit(points_per_block[0].size(), this->dof_handler.n_dofs(),
        this->dof_handler.max_couplings_between_dofs());
