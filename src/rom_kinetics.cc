...
template <int dim, int n_fe_degree>
  void ROMKinetics<dim, n_fe_degree>::init_time_computation ()
  {
    ...
    for (unsigned int p = 0; p < n_prec; p++)
      XBF[p].reinit(materials, matrixfree_type_time, p, listen_to_material_id);
 ...
 */
template <int dim, int n_fe_degree>
  void ROMKinetics<dim, n_fe_degree>::compute_pod_basis_LUPOD (
    std::vector<PETScWrappers::MPI::BlockVector> &_snapshots,
    double epsilon_M,
    double epsilon_N)
  {
...
template <int dim, int n_fe_degree>
  void ROMKinetics<dim, n_fe_degree>::assemble_matrices ()
  {
    ...
    V.reinit(materials, matrixfree_type_time, listen_to_material_id);
    ...
    F.reinit(materials, matrixfree_type_time, listen_to_material_id);
...
template <int dim, int n_fe_degree>
  void ROMKinetics<dim, n_fe_degree>::run ()
  {
    ...
    for (unsigned int rom = 0; rom < n_sets_snap; rom++)
    {
      ...
      if (LUPOD_flag)
      {
        compute_pod_basis_LUPOD(snapshots[rom], epsilon_M, epsilon_N);
      }
      else
      {
        compute_pod_basis(snapshots[rom]); // TODO implement epsilon_M
      }
