  ...
  std::vector<BlockVector<double> > &snap_basis_red)
{
  ...
  for (unsigned int b = 0; b < n_blocks; b++)
  {
    ...
    // Compute the SVD of matrix S_svd
    petsc_svd(S_svd, U_block);

    for (unsigned int k = 0; k < K; k++)
    {
      snap_basis_ful[b * K + k].block(b) = U_block[k];
      snap_basis_ful[b * K + k].compress(VectorOperation::insert);
    }
  ...
  std::vector<PETScWrappers::MPI::Vector> &U)
{
  ...
  AssertRelease(static_cast<unsigned int>(K) == U.size(),
    "Error in petsc_svd: U must be resized before calling petsc_svd");
  U.resize(K);
  ...
  {
    SVDGetSingularTriplet(svd, k_sv, &singular_value, u, NULL);
//    std::cout << "      Singular value " << k_sv << ": " << std::scientific
//              << singular_value
//              << std::fixed << std::endl;
    U[k_sv] = u;
  }
  ...
  std::vector<PETScWrappers::MPI::BlockVector> &snap_basis)
{
  ...
  std::vector<PETScWrappers::MPI::Vector> U_block(K, snapshots[0].block(0));
  for (unsigned int k = 0; k < K; k++)
    U_block[k].reinit(snapshots[0].block(0));
  ...
  // For each block Create and perform the SVD
  for (unsigned int b = 0; b < n_blocks; b++)
  {
    for (PetscInt k_snap = 0; k_snap < static_cast<int>(K); k_snap++)
    {
      ...
      MatSetValues(Mat_snap, J, idm, 1, &k_snap, values_snap, INSERT_VALUES);
    }
    MatAssemblyBegin(Mat_snap, MAT_FINAL_ASSEMBLY);
    MatAssemblyEnd(Mat_snap, MAT_FINAL_ASSEMBLY);

    // Perform SVD
    petsc_svd(Mat_snap, U_block);

    for (unsigned int k = 0; k < K; k++)
    {
      snap_basis[b * K + k].block(b) = U_block[k];
      snap_basis[b * K + k].compress(VectorOperation::insert);
    }
  }

  MatDestroy(&Mat_snap);

  // TODO SET AN ENERGY THRESHOLD (use epsilon_M)
  //    std::cout << std::fixed;
  //    // Determine the number of modes to retain based on epsilon_M
  //    double total_energy = Sigma.l2_norm();
  //    unsigned int M = 0;
  ...
  //    {
  //      double leftout_energy = 0.0;
  //      for (unsigned int m2 = m + 1; m2 < K; ++m2)
  //        leftout_energy += Sigma(m2) * Sigma(m2);
  //      leftout_energy = sqrt(leftout_energy);
  //
  //      if (leftout_energy / total_energy <= epsilon_M)
...
//      {
//        unique_vertex_dofs.insert(local_dof_indices[n_dofs_per_cell - v - 1]);
//      }
...
void test_LUPOD_extended ()
{
  ...
  std::vector<unsigned int> snaps2;
  std::vector<std::vector<unsigned int> > points_per_block2;
  unsigned int dim_rom;

  compute_LUPODext_basis_monolithic(S, epsilon_M, n_points, snaps2, points_per_block2,
    dim_rom, U_ful, U_red);
...
void test_LUPOD_extended_group_wise ()
{
  ...
  // Run the function
  compute_LUPODext_basis_group_wise(S, epsilon_M, n_points, snaps, points_per_block,
    dim_rom, U_ful, U_red);
...
/**
 * Just a test for compute_POD_basis_group_wise
 */
void test_POD_groupwise ()
{
  ...
  compute_POD_basis_group_wise(snapshots, epsilon_M, dim_rom, snap_basis);
