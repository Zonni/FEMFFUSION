 ...
 */
template <int dim, int n_fe_degree>
  void ROMKinetics<dim, n_fe_degree>::compute_pod_basis_LUPOD_monolithic (
    std::vector<PETScWrappers::MPI::BlockVector> &_snapshots)
  {
    unsigned int J = n_dofs * n_groups;
    unsigned int K = _snapshots.size();

    cout << "   LUPOD TRUE " << std::endl;
    //std::cout << " J " << J << "  K " << K << " SIZE " << _snapshots[0].size() << std::endl;
    // Create the matrix with the snapshot to apply the SVD
 ...
 */
template <int dim, int n_fe_degree>
  void ROMKinetics<dim, n_fe_degree>::compute_pod_basis_group_wise (
    std::vector<PETScWrappers::MPI::BlockVector> &)
  {
 ...
 */
template <int dim, int n_fe_degree>
  void ROMKinetics<dim, n_fe_degree>::compute_pod_basis_LUPOD_group_wise (
    std::vector<PETScWrappers::MPI::BlockVector> &)
  {
  }
 ...
 */
template <int dim, int n_fe_degree>
  void ROMKinetics<dim, n_fe_degree>::qr (
...
template <int dim, int n_fe_degree>
  void ROMKinetics<dim, n_fe_degree>::run ()
  {
    ...
    for (unsigned int rom = 0; rom < n_sets_snap; rom++)
    {
      ...
      {
        if (LUPOD_flag)
          compute_pod_basis_LUPOD_monolithic(snapshots[rom]);
        else
          compute_pod_basis_monolithic(snapshots[rom]);
      }
      else if (rom_group_wise == "Group_Wise")
      {
        if (LUPOD_flag)
          compute_pod_basis_LUPOD_group_wise(snapshots[rom]);
        else
          compute_pod_basis_group_wise(snapshots[rom]);
      }
      else
