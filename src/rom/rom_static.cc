  ...
  void ROMStatic<dim, n_fe_degree>::run ()
  {
    ...
    if (rom_group_wise == "Monolithic")
    {
      if (LUPOD_type == "LUPOD")
      {
        ...
        compute_LUPOD_basis_monolithic(snapshots, epsilon_M, epsilon_N,
          snaps, points_per_block, dim_rom, snap_basis_full, snap_basis_red);
      }
      ...
      else if (LUPOD_type == "POD")
      {
        ...
        compute_POD_basis_monolithic(snapshots, epsilon_M, snaps, dim_rom, snap_basis);
      }
    }
    else if (rom_group_wise == "Group_Wise")
    {
      ...
      else if (LUPOD_type == "Random")
      {
        ...
        compute_random_points_group_wise(snapshots, epsilon_M, n_LUPOD_points,
          snaps, points_per_block, dim_rom, snap_basis_full, snap_basis_red);
      }
      else if (LUPOD_type == "FEM1")
      {
        cout << "   LUPOD FEM 1 ----- GROUP WISE" << std::endl;
        compute_points_FEM1<dim>(dof_handler, snapshots, epsilon_M, n_LUPOD_points,
          snaps, points_per_block, dim_rom, snap_basis_full, snap_basis_red);
      }
    }
    else
    ...
    for (unsigned int t = 0; t < n_test; t++)
    {
      ...
      timer_eig_starts = timer.cpu_time();
      eig_rom[t] = solve_eigenvalue(phi_rom[t]);
      time_eig += timer.cpu_time() - timer_eig_starts;
      // Postprocess
    ...
    double time_rom = timer.cpu_time() - time_get_snap;
    ...
    for (unsigned int t = 0; t < n_test; t++)
    {
      ...
      static_problem.assemble_system_lambda();
      timer_fom_eig_start = timer.cpu_time();
      static_problem.solve_eps();
      time_fom_eig += timer.cpu_time() - timer_fom_eig_start;
    ...
    double time_fom = timer.cpu_time() - time_get_snap - time_rom;
