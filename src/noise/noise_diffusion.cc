/**
 *
 * @file  noise_diffusion.cc
 * @brief Implementation of the neutron noise diffusion equation with blocks.
 */
...
template <int dim, int n_fe_degree>
  NoiseDiffusion<dim, n_fe_degree>::NoiseDiffusion (ParameterHandler &prm,
    StaticDiffusion<dim, n_fe_degree> &static_problem,
    const bool verbose,
    const bool /*silent*/)
  :
      comm(static_problem.comm),
      verbose_cout(static_problem.verbose_cout),
      cout(static_problem.cout),
      n_groups(static_problem.n_groups),
      n_blocks(2 * static_problem.n_groups),
      ...
      timer(static_problem.timer)
  {
    ...
    AssertRelease(Utilities::MPI::n_mpi_processes(comm) == 1,
      "Diffusion Noise not implemented in Parallel");
    timer.restart();
    cout << std::endl;
    cout << "   ---------------------" << std::endl;
    ...
    phi_crit.reinit(n_blocks, comm, n_dofs, n_dofs);
    for (unsigned int g = 0; g < n_groups; g++)
      phi_crit.block(2 * g) = static_problem.phi[0].block(g);
 ...
 */
template <int dim, int n_fe_degree>
  void NoiseDiffusion<dim, n_fe_degree>::get_parameters_from_command_line ()
  {
 ...
 */
template <int dim, int n_fe_degree>
  void NoiseDiffusion<dim, n_fe_degree>::assemble_system ()
  {

    system_rhs.reinit(n_blocks, comm, n_dofs, n_dofs);
    delta_phi.reinit(n_blocks, comm, n_dofs, n_dofs);
    ...
    A.reinit(materials, pert, boundary_conditions, albedo_factors, matrixfree_type_A);
    verbose_cout << "  Done!" << std::endl;
    ...
    get_string_from_options("-print_matrices_noise_matlab", matrix_file);

    if (!matrix_file.empty())
      print_matrices(matrix_file);
 ...
 */
template <int dim, int n_fe_degree>
  void NoiseDiffusion<dim, n_fe_degree>::assemble_rhs ()
  {
    NoiseBMatrix<dim, n_fe_degree> B(comm, dof_handler_block, constraints);
    ...
    B.reinit(materials, pert, matrixfree_type_B);
    verbose_cout << "  Done!" << std::endl;
 ...
 *  with '-print_matrices_noise_matlab' command line option.
 */
template <int dim, int n_fe_degree>
  void NoiseDiffusion<dim, n_fe_degree>::print_matrices (const std::string &matrix_file)
  {

    AssertRelease(A.matrixfree_type == full_allocated,
      "-print_matrices_matlab must be used with -allocate_matrices option");

    std::ofstream out(matrix_file.c_str(), std::ios::out);

    for (unsigned int g1 = 0; g1 < n_blocks; g1++)
      for (unsigned int g2 = 0; g2 < n_blocks; g2++)
      {
        A.print_matrix_in_matlab(g1, g2,
          "A_" + num_to_str(g1 + 1) + num_to_str(g2 + 1), out, 8);
      }
    ...
    for (unsigned int g1 = 0; g1 < n_blocks; g1++)
    {
      for (unsigned int g2 = 0; g2 < n_blocks; g2++)
      {
        out << "A_" + num_to_str(g1 + 1) + num_to_str(g2 + 1) << " ";
      }
      out << ";" << std::endl;
    }
    out << "];";

    print_block_vector_in_matlab(system_rhs, "rhs", out, 8);
    out.close();
 ...
 */
template <int dim, int n_fe_degree>
  void NoiseDiffusion<dim, n_fe_degree>::solve ()
  {
    ...
    double alpha = 0.0;
    get_double_from_options("-alpha", alpha);
    complex_solver.alpha = alpha;
    cout << "      Complex preconditioner: " << complex_solver.pc_complex << std::endl;

    complex_solver.solve(system_rhs, delta_phi);
...
/**
 * @brief Postprocess and Print .out
 *
 */
template <int dim, int n_fe_degree>
  void NoiseDiffusion<dim, n_fe_degree>::postprocess ()
  {
    ...
    typename DoFHandler<dim>::active_cell_iterator endc = dof_handler_block.end();
    for (; cell != endc; ++cell)
    {
      ...
      for (unsigned int g = 0; g < n_groups; g++)
      {
        fe_values.get_function_values(delta_phi.block(2 * g), local_delta_phi[g][0]);
        fe_values.get_function_values(delta_phi.block(2 * g + 1), local_delta_phi[g][1]);
      }
      ...
      for (unsigned int q = 0; q < n_q_points; q++)
      {
        for (unsigned int g = 0; g < n_groups; g++)
        {
          delta_cell_phi[g][0] += local_delta_phi[g][0][q] * fe_values.JxW(q);
          delta_cell_phi[g][1] += local_delta_phi[g][1][q] * fe_values.JxW(q);
        }
      }
...
template <int dim, int n_fe_degree>
void
NoiseDiffusion<dim, n_fe_degree>::output_results_matlab () const
{
 ...
 */
template <int dim, int n_fe_degree>
  void NoiseDiffusion<dim, n_fe_degree>::output_results_vtk () const
  {
    ...
    for (unsigned g = 0; g < n_groups; g++)
    {
      ...
      {
        dphi[g][0] = delta_phi(2 * g * n_dofs + i);
        dphi[g][1] = delta_phi((2 * g + 1) * n_dofs + i);
        phi[g][i] = phi_crit(2 * g * n_dofs + i);
 ...
 */
template <int dim, int n_fe_degree>
  void NoiseDiffusion<dim, n_fe_degree>::output_detectors () const
  {
      ...
      for (unsigned int a = 0; a < n_assemblies; a++)
      {
            ...
            dt_rad++)
        {
          if (detectors_idx[dt_rad] == (a % n_assemblies_per_plane))
          {
            ...
            for (unsigned int dt_axl = 0; dt_axl < n_axial_dtc; dt_axl++)
            {
              if ((a_axl >= detector_levels[2 * dt_axl])
                  and (a_axl <= detector_levels[2 * dt_axl + 1]))
              {
                const unsigned int dtc = dt_rad * n_axial_dtc + dt_axl;
                noise_at_detectors[gr][dtc] += noise_per_assembly[gr][a]
 ...
 */
template <int dim, int n_fe_degree>
  void NoiseDiffusion<dim, n_fe_degree>::run ()
  {
    ...
    verbose_cout << "  assemble_system..." << std::endl;
    assemble_system();
    verbose_cout << "  Done! " << std::endl;

    cout << "      Number of DoFs per block: " << n_dofs << std::endl;
    cout << "      Number of Total DoFs: " << n_dofs * n_blocks << std::endl;
...
template class NoiseDiffusion<1, 1> ;
template class NoiseDiffusion<1, 2> ;
template class NoiseDiffusion<1, 3> ;
template class NoiseDiffusion<1, 4> ;
template class NoiseDiffusion<1, 5> ;

template class NoiseDiffusion<2, 1> ;
template class NoiseDiffusion<2, 2> ;
template class NoiseDiffusion<2, 3> ;
template class NoiseDiffusion<2, 4> ;
template class NoiseDiffusion<2, 5> ;

template class NoiseDiffusion<3, 1> ;
template class NoiseDiffusion<3, 2> ;
template class NoiseDiffusion<3, 3> ;
template class NoiseDiffusion<3, 4> ;
template class NoiseDiffusion<3, 5> ;
