 ...
 *
 * @file   static_sdpn.cc
 * @brief  A program to solve static neutron Simplified Doubles PN equation with the finite element method.
 */
...
/**
 * @brief Constructor of the main class StaticSDPN.
 * Reads the input file and it reads or builds the grid.
 */
template <int dim, int n_fe_degree>
  StaticSDPN<dim, n_fe_degree>::StaticSDPN (ParameterHandler &prm,
 ...
 */
template <int dim, int n_fe_degree>
  void StaticSDPN<dim, n_fe_degree>::get_mesh_shape (ParameterHandler &prm)
  {
 ...
 */
template <int dim, int n_fe_degree>
  void StaticSDPN<dim, n_fe_degree>::make_rectangular_grid ()
  {
 ...
 */
template <int dim, int n_fe_degree>
  void StaticSDPN<dim, n_fe_degree>::get_unstructured_grid (
 ...
 */
template <int dim, int n_fe_degree>
  void StaticSDPN<dim, n_fe_degree>::get_parameters_from_command_line ()
  {
    // Booleans
    ...
    get_string_from_options("-sdpn_type", sdpn_type);
 ...
 */
template <int dim, int n_fe_degree>
  void StaticSDPN<dim, n_fe_degree>::make_dofs ()
  {
 ...
 */
template <int dim, int n_fe_degree>
  void StaticSDPN<dim, n_fe_degree>::assemble_system ()
  {
    define_sdpn_coeffs(sdpn_type, n_moments);
 ...
 */
template <int dim, int n_fe_degree>
  PetscErrorCode StaticSDPN<dim, n_fe_degree>::print_matrices ()
  {
 ...
 */
template <int dim, int n_fe_degree>
  void StaticSDPN<dim, n_fe_degree>::solve_eps ()
  {
 ...
 */
template <int dim, int n_fe_degree>
  void StaticSDPN<dim, n_fe_degree>::postprocess ()
  {
    std::vector<std::vector<double> > sdpn_u_to_phi_coeff = get_sdpn_u_to_phi_coeff(
      sdpn_type, n_moments);

    std::cout << "n_moments " << n_moments << std::endl;
    print_vector(sdpn_u_to_phi_coeff[0]);
    ...
    for (unsigned int m1 = 0; m1 < n_moments; ++m1)
    {
      for (unsigned int eig = 0; eig < n_eigenvalues; eig++)
      {
        ...
        for (unsigned int g = 0; g < n_groups; ++g)
        {
          for (unsigned int m = 0; m < n_moments; ++m)
          {
            phi[m1][eig].block(g).add(sdpn_u_to_phi_coeff[m1][m],
...
template <int dim, int n_fe_degree>
  void
  StaticSDPN<dim, n_fe_degree>::output_to_noise ()
  {
 ...
 */
template <int dim, int n_fe_degree>
  void StaticSDPN<dim, n_fe_degree>::run ()
  {
    ...
    cout << "   Grid Done." << " Time = " << timer.cpu_time() << " s."
         << std::endl;
    cout << "      Equations:  SDP" << n_moments - 1 << std::endl;
...
template class StaticSDPN<1, 1> ;
template class StaticSDPN<1, 2> ;
template class StaticSDPN<1, 3> ;
template class StaticSDPN<1, 4> ;
template class StaticSDPN<1, 5> ;

template class StaticSDPN<2, 1> ;
template class StaticSDPN<2, 2> ;
template class StaticSDPN<2, 3> ;
template class StaticSDPN<2, 4> ;
template class StaticSDPN<2, 5> ;

template class StaticSDPN<3, 1> ;
template class StaticSDPN<3, 2> ;
template class StaticSDPN<3, 3> ;
template class StaticSDPN<3, 4> ;
template class StaticSDPN<3, 5> ;
